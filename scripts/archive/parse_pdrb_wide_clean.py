"""
parse_pdrb_wide_clean.py — Wide-format variant with clean-at-parse behavior.

Values are cleaned by Claude before output (stray chars removed, splits collapsed).
Contrast with parse_pdrb_wide.py which captures values verbatim (faithful-snapshot).
Use this variant to compare output quality against the verbatim version.
...
- values: exactly as printed, preserving Indonesian formatting (period = thousands
  separator, comma = decimal); do not round or reformat; empty if cell is blank
...
def melt_to_long(headers_df: pd.DataFrame, data_df: pd.DataFrame) -> list[dict]:
    ...
    # detect year columns: 4-digit number optionally followed by * or **
    year_pattern = re.compile(r"^\d{4}\*{0,2}$")
    ...
    melted["year_flag"] = melted["year_raw"].str.extract(r"(\*+)$").fillna("")
    melted["value_flag"] = ""
...
def main():
    ...
    for pdf_path in pdfs:
        ...
        try:
            total_rows, anomalies = parse_pdf(pdf_path, client, csv_out, done, args.max_pages)
