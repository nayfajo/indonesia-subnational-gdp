"""
parse_pdrb_wide_b.py — Group B variant for 2000-2007 BPS PDFs.

Differs from parse_pdrb_wide.py (Group C) in two ways:
  1. Pipe-delimited output instead of comma-delimited: 2000-2007 PDFs use comma as
     decimal separator (e.g. 138.801,52), which collides with CSV commas and shifts
     all columns right. Pipe avoids this entirely.
  2. province_name extracted from the table header (HEADERS section), not from data
     rows. In 2000-2007 publications the province name appears only in the table title,
     not as a repeated column in the district rows.

All other logic — faithful-snapshot principle, wide-to-long melt, auto mode detection,
--resume, --max-pages, --force-vision — is identical to parse_pdrb_wide.py.

Applies to:
    PDRB_2000-2003.pdf, PDRB_2003-2007.pdf
    PDRB_capita_2000-2003.pdf, PDRB_capita_2003-2007.pdf (once main series validated)

Usage:
    .venv/bin/python3.14 scripts/parse_pdrb_wide_b.py \\
        --input raw/pdf/PDRB_2000-2003.pdf \\
        --output pipeline_out/trial_b \\
        --max-pages 20
...
- province_name: province name extracted from the table title
  (e.g. "PDRB Kabupaten/Kota di Propinsi Aceh Atas Dasar Harga Berlaku" → "ACEH");
...
- name_flag: verbatim trailing marker on region name; strip it from region_name_raw
  ...
  "1)" in the source PDF but pdfplumber text extraction mangles it unpredictably
    ...
    def read_csv(lines):
        ...
        try:
            return pd.read_csv(
                ...
                sep="|",
                dtype=str,
                quotechar="\x00",       # disable quote handling — stray " in values
...
def next_csv_path(csv_dir: Path, stem: str) -> Path:
    base = csv_dir / f"{stem}_wide_b.csv"
    if not base.exists():
        return base
    n = 2
    while True:
        candidate = csv_dir / f"{stem}_wide_b_{n}.csv"
...
def main():
    parser = argparse.ArgumentParser(description="Parse PDRB PDFs — Group B (2000-2007) pipe-delimited variant.")
    ...
    for pdf_path in pdfs:
        ...
        if args.resume:
            csv_out = csv_dir / f"{stem}_wide_b.csv"
