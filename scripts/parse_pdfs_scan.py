"""
parse_pdfs_scan.py — Wide-format parser for Group A BPS PDRB PDFs (1996-1999).

Group A PDFs are tilted physical scans. Two layout features differ from Groups B and C:
  1. Staggered rows — due to scan tilt, each district name appears on one line and
     its year values appear staggered on the line immediately below. The prompt
     explicitly instructs Claude to pair them as a single row.
  2. Comma decimal — values use comma as decimal separator (e.g. 647.980,52).
     Pipe delimiter (inherited from parse_pdfs_text.py) avoids the CSV field collision.

All other logic is identical to parse_pdfs_text.py: pipe delimiter always,
province_name in HEADERS, faithful-snapshot principle, wide-to-long melt, auto mode
detection (has_doubled_text routes 1996-1999 scans to vision automatically), --resume,
--max-pages, --force-vision, --debug, temperature=0, streaming API, incremental CSV writes.
...
─────────────────────────────────────────
IMPORTANT — staggered layout: In this PDF, due to scan tilt, each district name
appears on one line and its year values appear staggered on the line immediately
below it. Treat the district name and its staggered values together as a single
data row. Do not emit a separate row for the staggered values alone.
...
- year columns: use the exact year headers from the PDF including any footnote markers
  exactly as printed (e.g. 1996, 1997, 1998*), 1999**)); one column per year in the table
- values: exactly as printed — including comma decimal separators (e.g. 647.980,52),
...
cases where staggered row pairing was ambiguous
...
def main():
    parser = argparse.ArgumentParser(description="Parse Group A BPS PDRB PDFs (1996-1999 tilted scan).")
